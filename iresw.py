...
class IreswSkillAgent(BaseAgent):
    ...
    def __init__(self, cfg, ob_space, ac_space):
        ...
        self._use_amp = cfg.precision == 16
        self._dtype = torch.float16 if self._use_amp else torch.float32
        self._horizon_decay = LinearDecay(1, cfg.n_skill / 2, cfg.horizon_step)
        self._update_iter = 0
        self._std_decay = LinearDecay(cfg.max_std, cfg.min_std, cfg.std_step)

        self._ac_dim = ac_dim = gym.spaces.flatdim(self._ac_space)
        hidden_dims = [cfg.num_units] * cfg.num_layers
        self.action_wld = TDMPCModel(cfg, ob_space, self._ac_dim, self._dtype)
        self.action_wld_target = TDMPCModel(cfg, ob_space, self._ac_dim, self._dtype)
        copy_network(self.action_wld_target, self.action_wld)
    ...
    def state_dict(self):
        return {
            ...
            "action_wld": self.action_wld.state_dict(),
            "action_wld_target": self.action_wld_target.state_dict(),
            "skill_encoder": self.skill_encoder.state_dict(),
            "ob_norm": self._ob_norm.state_dict(),
            "obs_decoder": self.decoder.state_dict(),
        }

    def load_state_dict(self, ckpt):
        self.skill_decoder.load_state_dict(ckpt["skill_decoder"])
        self.action_wld.load_state_dict(ckpt["action_wld"])
        self.action_wld_target.load_state_dict(ckpt["action_wld_target"])
        self.skill_encoder.load_state_dict(ckpt["skill_encoder"])
        self.decoder.load_state_dict(ckpt["obs_decoder"])
        self.to(self._device)
    ...
    def act(self, ob, mean=None, cond=None, is_train=True, warmup=False):
        ...
        with torch.autocast(self._cfg.device, enabled=self._use_amp):
            ...
            if self._cfg.phase == "pretrain" or warmup or not self._cfg.use_cem:
                ...
                action_tensor, mean = self.plan(feat, cond, mean, is_train)
    ...
    @torch.no_grad()
    def plan(self, state, cond, prev_mean=None, is_train=True):
        """Plan given a state and conditioning via CEM over action trajectories."""
        cfg = self._cfg

        h = int(self._horizon_decay(self._step))

        num_policy = int(cfg.num_policy_traj / 2)
        num_sample = int(cfg.num_sample_traj / 2)
        num_tot = num_policy + num_sample

        z = state.repeat(num_policy, 1)  # [num_policy, state_dim]
        c_z = cond.repeat(num_policy, 1)  # [num_policy, cond_dim]

        skills = []
        for t in range(h):
            skill_t = self.skill_decoder.act(z, c_z, deterministic=True)  # [num_policy, ac_dim]
            skills.append(skill_t)
            z, _ = self.action_wld.imagine_step(z, skill_t)
        skill0 = torch.stack(skills, dim=0)  # [h, num_policy, ac_dim]

        # 4. Initialize CEM parameters
        mean = torch.zeros(h, self._ac_dim, device=state.device)
        std = torch.full_like(mean, 2.0)
        if prev_mean is not None and h == prev_mean.size(0) and h > 1:
            mean[:-1].copy_(prev_mean[1:])

        # 5. Preallocate sample tensor
        sample_action = torch.empty(h, num_sample, self._ac_dim, device=state.device)

        # 6. Prepare initial states/conditions for evaluation
        z0 = state.repeat(num_tot, 1)  # [num_tot, state_dim]
        c_z1 = cond.repeat(num_tot, 1)  # [num_tot, cond_dim]

        # 7. CEM optimization loop (with function2's global‐elite selection & weighting)
        for _ in range(cfg.cem_iter):
            # 7.1 Sample candidates
            sample_action.normal_()
            sample_action.mul_(std.unsqueeze(1)).add_(mean.unsqueeze(1))
            sample_action.clamp_(-0.999, 0.999)

            # 7.2 Form full set of trajectories
            actions = torch.cat([sample_action, skill0], dim=1)  # [h, num_tot, ac_dim]

            # 7.3 Evaluate returns
            returns = self.estimate_value(z0, actions, h, c_z1).squeeze(-1)  # [h, num_tot]

            # 7.4 Flatten for global elite selection
            flat_returns = returns.view(-1)  # [h * num_tot]
            flat_skills = actions.view(-1, self._ac_dim)  # [h * num_tot, ac_dim]

            # pick top‐cfg.num_elites across all timesteps & trajectories
            idxs = torch.topk(flat_returns, cfg.num_elites, dim=0).indices  # [E]
            elite_val = flat_returns[idxs]  # [E]
            elite_ac = flat_skills[idxs]  # [E, ac_dim]

            # 7.5 Compute global weights (function2 style)
            weights = torch.exp(cfg.cem_temperature * (elite_val - elite_val.max()))  # [E]
            weights = weights / weights.sum()  # [E]
            w = weights.view(1, -1, 1)  # [1, E, 1]

            # replicate elite actions across all timesteps
            elite_ac = elite_ac.expand(h, -1, self._ac_dim)  # [h, E, ac_dim]

            # 7.6 Update mean & std
            new_mean = (w * elite_ac).sum(dim=1)  # [h, ac_dim]
            new_std = torch.sqrt((w * (elite_ac - new_mean.unsqueeze(1)) ** 2).sum(dim=1))
            mean = cfg.cem_momentum * mean + (1 - cfg.cem_momentum) * new_mean
            std = torch.clamp(new_std, self._std_decay(self._step), 2.0)

        # 8. Sample final action
        probs = weights.cpu().numpy()  # [E]
        idx_choice = np.random.choice(cfg.num_elites, p=probs)
        selected_action = elite_ac[0, idx_choice]  # [ac_dim]
        if is_train:
            selected_action = selected_action + std[0] * torch.randn_like(std[0])

        return selected_action.clamp(-0.999, 0.999), mean

    @torch.no_grad()
    def estimate_value(self, state, ac, horizon, cond):
        """Imagine a trajectory for `horizon` steps, and estimate the value."""
        value, discount = 0.0, 1.0
        for t in range(horizon):
            state, reward = self.action_wld.imagine_step(state, ac[t])
            value += discount * reward
            discount *= self._cfg.rl_discount
        value += discount * torch.min(*self.action_wld.critic(state, self.skill_decoder.act(state, cond)))
        return value
...
class IreswAgent(BaseAgent):
    ...
    def _build_optims(self):
        ...
        self.ll_modules = [ll_agent.action_wld, ll_agent.skill_decoder, ll_agent.skill_encoder, ll_agent.decoder]
        ...
        if cfg.joint_training:
            ...
            self.ll_model_optim = adam_amp(
                [ll_agent.action_wld, ll_agent.decoder], cfg.model_lr
            )
            self.ll_actor_optim = adam_amp(ll_agent.skill_decoder, ll_agent.skill_encoder)

        if cfg.phase == "rl":
            actor_modules = [hl_agent.skill_prior]
            model_modules = [hl_agent.skill_wld, hl_agent.obs_decoder]
            ll_model_modules = [ll_agent.action_wld, ll_agent.decoder]
            if cfg.sac:
            ...
            self.ll_dmodel_optim = adam_amp(ll_model_modules, cfg.model_lr)
    ...
    def state_dict(self):
        ...
        if self._cfg.joint_training:
            ...
            ret["ll_model_optim"] = self.ll_model_optim.state_dict()
        ...
        if self._cfg.phase == "rl":
            ...
            ret["ll_dmodel_optim"] = self.ll_dmodel_optim.state_dict()
    ...
    def load_state_dict(self, ckpt):
        ...
        if self._cfg.joint_training:
            ...
            self.ll_model_optim.load_state_dict(ckpt["ll_model_optim"])
        ...
        if "model_optim" in ckpt:
            ...
            self.ll_dmodel_optim.load_state_dict(ckpt["ll_dmodel_optim"])
    ...
    def update(self):
        ...
        for _ in range(train_iter):
            ...
            ll_batch = self.ll_buffer.sample(self._cfg.batch_size)
            sw_data.stop()

            sw_train.start()
            hl_train_info = self._update_hl_network(hl_batch)
            ll_train_info = self._update_ll_network(ll_batch)
            train_info.add(hl_train_info)
            train_info.add(ll_train_info)
            sw_train.stop()
            ...
            for t in range(cfg.n_skill):
                ...
                with torch.no_grad():
                    ...
                    intrinsic_reward = rho * (
                            cfg.error_weight * state_error + cfg.transition_weight * state_transition).clamp(max=1e3)*0.5
    ...
    def _update_ll_network(self, batch):
        """Updates the low-level dynamics world_model and policy."""
        cfg = self._cfg
        info = Info()
        mse = nn.MSELoss(reduction="none")
        ll_agent = self.skill_agent

        # Optionally freeze encoder and dynamics
        if cfg.freeze_model:
            ll_agent.action_wld.encoder.requires_grad_(False)
            ll_agent.action_wld.dynamics.requires_grad_(False)
        ...
        # Encode observations
        ll_feat = flip(ll_agent.action_wld.encoder(o))
        with torch.no_grad():
            ll_feat_target = flip(ll_agent.action_wld_target.encoder(o))
        ...
        # Dynamics rollouts and loss accumulation
        for t in range(cfg.n_skill):
            z = ll_feat[t]
            # Predict next latent and reward
            z_next_pred, reward_pred = ll_agent.action_wld.imagine_step(z, ll_ac[t])
            # Select state for critic
            z_post = ob[t]["ob"] if cfg.sac else z_next_pred
            # Critic prediction
            q_pred = ll_agent.action_wld.critic(z_post, ll_ac[t])

            # Compute targets (no grad)
            with torch.no_grad():
                z_next = ll_feat_target[t + 1]
                q_next = torch.min(*ll_agent.action_wld_target.critic(z_next, ll_ac[t]))
                q_target = (
                        reward_pred.squeeze(-1)
                        + (1 - done_seq[t].long()) * cfg.rl_discount * q_next
                )

            # Accumulate world_model losses
            consistency_loss += mse(z_next_pred, z_next).mean(dim=1)
            reward_loss += mse(reward_pred.squeeze(-1), rew_seq[t].squeeze(-1))
            value_loss += mse(q_pred[0], q_target) + mse(q_pred[1], q_target)

        # Aggregate and normalize world_model losses
        ...
        # Model update
        model_loss = (
                cfg.scalars.consistency * consistency_loss
                + cfg.scalars.ll_reward * reward_loss
                + cfg.scalars.ll_value * value_loss
        )
        model_grad_norm = self.ll_dmodel_optim.step(model_loss)

        # Soft update target network
        soft_copy_network(ll_agent.action_wld, ll_agent.action_wld_target, cfg.target_update_tau)

        # Log info
        info["ll_consistency_loss"] = consistency_loss.item()
        info["ll_reward_loss"] = reward_loss.item()
        info["ll_value_loss"] = value_loss.item()
        info["ll_model_loss"] = model_loss.item()
        info["ll_model_grad_norm"] = model_grad_norm.item()
    ...
    def pretrain(self):
        ...
        Logger.info(
            f"[LL] skill_prior loss: {info['ll_actor_loss']:.3f}  world_model loss: {info['ll_model_loss']:.3f}  kl loss: {info['ll_vae_kl_loss']:.3f}"
        )
        return info
    ...
    def _pretrain(self, batch, is_train=True):
        ...
        with torch.autocast(self._cfg.device, enabled=self._use_amp):
            ...
            # update the obs encoder and decoder in low level
            ll_ob_pred = ll_agent.decoder(ll_feat)
            ll_recon_losses = {k: -ll_ob_pred[k].log_prob(flip(v)).mean() for k, v in o.items()}
            ll_recon_loss = sum(ll_recon_losses.values())

            ll_consistency_loss = 0.0
            hl_consistency_loss = 0.0

            for t in range(L):
                ac = ll_ac[t] if cfg.joint_training else ll_ac[t].detach()
                ll_next_pred, _ = ll_agent.action_wld.imagine_step(ll_feat[t], ac)
                ll_next_target = ll_feat_target[t + 1]
                ll_consistency_loss += (scalars.rho ** t) * mse(ll_next_pred, ll_next_target).mean(dim=1)
            ...
            ll_model_loss = scalars.ll_model * ll_recon_loss + scalars.consistency * ll_consistency_loss.clamp(
                max=1e4).mean()
            ll_model_loss.register_hook(lambda grad: grad * (1 / L))
        ...
        ll_loss = ll_actor_loss + ll_vae_kl_loss + ll_model_loss 
        ...
        info["ll_model_loss"] = ll_model_loss.item()
        ...
        info["ll_consistency_loss"] = ll_consistency_loss.mean().item()
        ...
        if cfg.joint_training:
            ...
            ll_model_grad_norm = self.ll_model_optim.step(ll_model_loss)
            ...
            info["ll_model_grad_norm"] = ll_model_grad_norm.item()

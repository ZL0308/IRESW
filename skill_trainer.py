...
from skimo.rolf.trainer import Trainer
from skimo.rolf.utils import Logger, LOG_TYPES, Every, StopWatch, Info
from skimo.rolf.utils.pytorch import to_tensor
...
class SkillTrainer(Trainer):
    ...
    def _get_agent_by_name(self, algo):
        if algo == "spirl_dreamer":
            from skimo.spirl_dreamer_agent import SPiRLDreamerAgent

            return SPiRLDreamerAgent
        if algo == "skimo":
            from skimo.skimo_agent import SkiMoAgent

            return SkiMoAgent
        if algo == "iresw":
            from iresw import IreswAgent
    ...
    def _visualize_value(self, step):
        ...
        feat = self._agent.meta_agent.skill_wld.encoder(ob_tensor)
        ac = self._agent.meta_agent.skill_prior(feat).mode()
        value = (
            torch.min(*self._agent.meta_agent.skill_wld.critic(feat, ac))
            .detach()
            .cpu()
            .numpy()
        )
